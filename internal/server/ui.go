...
async function load(){var r=await fetch(A+'/backups').then(function(r){return r.json()});items=r.backups||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.source||'').toLowerCase().includes(q)||(i.destination||'').toLowerCase().includes(q)||(i.status||'').toLowerCase().includes(q)});
...
if(i.destination)h+='<span>'+esc(i.destination)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/backups/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",source:"",destination:"",size_bytes:0,status:"",schedule:"",last_run_at:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' SADDLEBAG</h2>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),source:document.getElementById('f-source').value.trim(),destination:document.getElementById('f-destination').value.trim(),size_bytes:parseInt(document.getElementById('f-size-bytes').value)||0,status:document.getElementById('f-status').value.trim(),schedule:document.getElementById('f-schedule').value.trim(),last_run_at:document.getElementById('f-last-run-at').value.trim()};
if(editId){await fetch(A+'/backups/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/backups',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
